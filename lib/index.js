...
function nic_by_name(name, cb) {
  ...
  os_functions[process.platform].get_network_interfaces_list(function(err, list) {
...
exports.get_private_ip = function(cb) {
  ...
  os_functions[process.platform].get_network_interfaces_list(function(err, list) {
    ...
    os_functions[process.platform].get_active_network_interface_name(function(err, active_nic) {
      if (err) return cb(err);
...
exports.get_gateway_ip = function(cb) {
  ...
  os_functions[process.platform].get_active_network_interface_name(function(err, nic_name) {
    if (err || nic_name.trim() == '')
      return cb(err || new Error('No active network interface found.'));

    os_functions[process.platform].gateway_ip_for(nic_name, function(err, out) {
...
exports.get_active_interface = function(cb) {
  ...
  os_functions[process.platform].get_active_network_interface_name(function(err, nic_name) {
    ...
    nic_by_name(nic_name, function(err, nic) {
      ...
      os_functions[process.platform].netmask_for(nic_name, function(err, netmask) {
        if (!err && netmask)
          nic.netmask = netmask.trim();

        os_functions[process.platform].gateway_ip_for(nic_name, function(err, ip) {
          if (!err && ip)
...
exports.get_interfaces_list = os_functions[process.platform].get_network_interfaces_list;
exports.mac_address_for = os_functions[process.platform].mac_address_for;
